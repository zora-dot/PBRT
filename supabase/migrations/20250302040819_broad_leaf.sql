...
-- Add missing columns to follows table
ALTER TABLE follows 
ADD COLUMN IF NOT EXISTS past_id uuid DEFAULT gen_random_uuid(),
ADD COLUMN IF NOT EXISTS post_id uuid DEFAULT gen_random_uuid();

-- Create unique constraint on follower_id and following_id if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
