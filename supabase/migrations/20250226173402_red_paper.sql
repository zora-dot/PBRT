...
-- Create a partial index for auto-saved drafts
DROP INDEX IF EXISTS idx_unique_user_autosaved_draft;
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_user_autosaved_draft
ON drafts (user_id)
WHERE is_auto_saved = true;

-- Update save_draft function to handle conflicts better
...
-- Update get_user_drafts function with better error handling
