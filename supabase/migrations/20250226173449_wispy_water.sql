...
CREATE OR REPLACE FUNCTION check_login_exists_v2(
  p_login TEXT
)
RETURNS TABLE (
  user_exists BOOLEAN,
...
CREATE INDEX IF NOT EXISTS idx_users_email_lookup_v2 
ON auth.users(LOWER(email)) 
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_profiles_username_lookup_v2 
ON public.profiles(LOWER(username));

-- Grant necessary permissions
GRANT EXECUTE ON FUNCTION check_login_exists_v2(TEXT) TO anon, authenticated;

-- Add helpful comments
COMMENT ON FUNCTION check_login_exists_v2 IS 
